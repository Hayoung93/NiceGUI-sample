...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None, da=False):
        ...
        if da:
            ...
            mem_split = memory.permute(0, 2, 1).split(sizes, dim=-1)
            feats = []
            for shapes, mem in zip(spatial_shapes, mem_split):
                feats.append(mem.view(bs, c, shapes[0], shapes[1]))
            da_meta.append(feats)
        else:
            da_meta = None
        ...
        if self.two_stage:
            return hs, init_reference_out, inter_references_out, enc_outputs_class, enc_outputs_coord_unact, da_meta
        return hs, init_reference_out, inter_references_out, None, None, da_meta
...
class DeformableDETR(nn.Module):
    ...
    def forward(self, samples, da=False):
        ...
        return self.forward_samples(samples, da=da)

    def forward_samples(self, samples, da=False):
        ...
        hs, init_reference, inter_references, enc_outputs_class, enc_outputs_coord_unact, da_features = self.transformer(srcs, masks, pos, query_embeds, da=da)
        outputs_classes = []
        outputs_coords = []
        outputs_features = []
